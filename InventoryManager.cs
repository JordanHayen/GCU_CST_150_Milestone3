...
namespace Milestone3
{
    class InventoryManager
    {
        ...
        public bool Search(string searchTerm, out List<int> indecesFound, out List<InventoryItem> itemsFound)
        {
            ...
            // LINQ is used to search the Items List property. The name of each InventoryItem is compared to the searchTerm parameter using the Contains method. All strings are set to lower case to ensure case independency. The result is an object that contains all found objects
            var itemNameQuery = from item in this.Items
                                where item.ItemName.ToLower().Contains(searchTerm.ToLower())
                                select item;
            // LINQ is used to search the Items List property. The name of each InventoryItem is compared to the searchTerm parameter using the Contains method. All strings are set to lower case to ensure case independency. The result is an object that contains the indeces of all found objects
